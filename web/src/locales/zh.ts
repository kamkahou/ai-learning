  ...
  translation: {
    common: {
      ...
      llmConfigReminder: {
        title: '需要配置LLM模型',
        alertTitle: '配置提醒',
        alertDescription: '请配置至少一个LLM模型以启用所有用户的聊天功能。',
        description:
          '作为管理员，您需要先配置LLM设置，用户才能开始聊天。一旦您配置了LLM模型，所有用户都能继承这些设置并使用聊天功能。',
        instruction: '您可以在用户设置 > 模型配置中配置LLM模型。',
        later: '稍后',
        goToSettings: '前往设置',
      },
    },
...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    fileManager: {
      ...
      uploadTitle: '点击或拖拽文件至此区域即可上传',
      uploadDescription:
        '支持单次或批量上传。每次上传的总文件大小限制为 30MB，每个文件不得超过 10MB，每个账户最多可上传 3 个文件。严禁上传违禁文件。',
      ...
      preview: '预览',
      fileError: '文件错误',
      uploadLimit: '文件大小不能超过10M，文件总数不超过3个',
      uploadLimitAdmin: '单次批量上传文件数不超过32，单个账户不限文件数量',
